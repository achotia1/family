$(document).ready(function(){
	...
	// console.log(live_lecture_ids);
    var state_id=$("#online-landing").selectbox().val();
	getCreditData(landing_id,live_lecture_ids,state_id);
...
function getCreditData(id,live_lecture_ids, state_id) {
    
    const action = SITEURL + '/live-lecture/landing_credit_types?id=' + id 
    			+ '&live_lecture_id=' + live_lecture_ids+ '&state_id=' + state_id+ '&multiday=1';
    ...
    axios.post(action)
        .then(function (response) {
            ...
            if (resp.status == 'success') {
                ...
                $("#live_cle_details").html(resp.live_lectures_string);
