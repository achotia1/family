...
{
    var action = ADMINURL+'/vehicles/getRecords'; 
    ...
    const table = $('#listingTable').DataTable( 
    {
        ...
        "ajax": {
            ...
            {
                object.custom = {
                    "chassis_number" :  $('#chassis-number').val(),
...
function deleteCollections(element)
{
   ...
   if ($members.length == 0) 
   {
      swal("Error",'Please select atleast one vehicle.','error');
      return false; 
   }
   else
   {
      ...
      action = ADMINURL+'/vehicles/bulkDelete';
