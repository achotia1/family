...
<table border="0" cellspacing="0" cellpadding="0" style="background:#eaeaea; max-width:800px; width:100%;padding:0px 15px;" align="center">
  ...
  <tr>
    <td bgcolor="#ffffff" align="center">
        ...
        <tr>
          <td style=" background-image:url({{  asset('assets/web/images/header-bg.png') }}; min-height: 185px; color: #ffffff; text-align: center; font-size: 26px; line-height: 38px; background-size: 100% auto; background-repeat: no-repeat; padding:7% 0; background-color:#667ab8;"><p style="margin:0; color:#ffffff;">Welcome to {{ $company->name??"" }} , </p></td>
        </tr>
        @if($company->mail_type=='user')
          <tr>
            <td style=""><h2 style="text-align: center; font-size:18px;text-transform: uppercase;">Welcome to {{ $company->name ?? 'Company' }}</h2></td>
          </tr>
        @endif
        <tr>
          <td><table align="center" width="100%" border="0" cellspacing="0" cellpadding="0" style="">
              <tr>
                <td>
                  @if($company->mail_type=='user')
                    Hello {{ $company->contact_name }}, <br/><br/> Below Note is added for your Order No: {{ $company->order_number??'' }}
                  @else
                    Hello Admin, <br/><br/> Below Note is added for Order No: {{ $company->order_number }}
                  @endif
                  <br/><br/>
                    Note : {{ $company->note??'' }}
                </td>
           ...
           {{ $company->name??"" }} <br>
          <a target="_blank" href="{{  $company->url??'javascript:void(0)' }}">{{  $company->url??"" }}</a><br>
              </td>
              </tr>
